...
ls apps/server/.env apps/web/.env 2>/dev/null                     # Env files created by `pnpm setup`?
...
The script is a **one-shot** — running it again after rename is a no-op. After rename, the engineer must commit the changes before `pnpm setup`, otherwise Postgres / Render / Vercel resources will be named with the placeholder.
...
### After doc Step 3 (`pnpm setup`)

`pnpm setup` is the **single first-run bootstrap**. It does four things in sequence:
...
The `\dt` output should list at least `users` and `files` plus the migration tracking table. If it shows no relations, migrations didn't apply — re-run `pnpm setup`.
...
**MinIO bucket missing:** If `pnpm setup` finished but the `file-uploads` bucket isn't there, the `minio-init` container errored. Re-run `docker compose run --rm minio-init` from the repo root — it's gated behind the `init` profile so a plain `pnpm infra:up` won't re-trigger it.

**`pnpm setup` is idempotent** — re-running is always safe. Day-to-day after first setup, only `pnpm infra:up` (start containers) and `pnpm dev` (start app) are needed.
