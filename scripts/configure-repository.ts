...
const enableSecurityFeatures = async (): Promise<"enabled" | "unavailable"> => {
	const result =
		await $`gh api --method PATCH ${`/repos/${REPO}`} --field ${"security_and_analysis[secret_scanning][status]=enabled"} --field ${"security_and_analysis[secret_scanning_push_protection][status]=enabled"}`.nothrow();
	if (result.exitCode === 0) return "enabled";
	const text = String(result.stderr || result.stdout || "");
	if (
		/Advanced Security is not available|secret scanning is not available/i.test(
			text,
		)
	)
		return "unavailable";
	mapApiError("enable security features", text);
	return "enabled";
};
...
note(
	[
		...
		"  4. Enable secret scanning + push protection (where supported)",
		"  5. Enable Dependabot vulnerability alerts and security fixes",
	].join("\n"),
	"Plan",
);
...
await runStep(
	"[1/5] Applying merge settings",
	() => "Merge settings applied",
	setMergeSettings,
);

await runStep(
	"[2/5] Ensuring 'main' and 'develop' branches exist",
...
const securityResult = await runStep(
	"[4/5] Enabling secret scanning + push protection",
	(r) => (r === "enabled" ? "Enabled" : "Not available on this plan (skipped)"),
	enableSecurityFeatures,
);
if (securityResult === "unavailable") {
	log.warn(
		"Private repos need GitHub Advanced Security; public repos get both free.",
	);
}

await runStep(
	"[5/5] Enabling Dependabot alerts + automated fixes",
	() => "Dependabot enabled",
	enableDependabot,
);
